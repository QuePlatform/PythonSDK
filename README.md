...
**Inherit from [`QueError`](./src/que/errors/queerror.py)**:
* [`ResponseValidationError`](./src/que/errors/responsevalidationerror.py): Type mismatch between the response data and the expected Pydantic model. Provides access to the Pydantic validation error via the `cause` attribute.
