"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
import httpx
from que.errors import QueError
from que.models import problemresponse_error as models_problemresponse_error
from que.types import BaseModel, OptionalNullable, UNSET
from typing import Optional


class ProblemResponseErrorData(BaseModel):
    type: str

    title: str

    status: int

    code: str

    detail: OptionalNullable[str] = UNSET

    details: OptionalNullable[models_problemresponse_error.Details] = UNSET


class ProblemResponseError(QueError):
    r"""An RFC 7807 problem details response."""

    data: ProblemResponseErrorData

    def __init__(
        self,
        data: ProblemResponseErrorData,
        raw_response: httpx.Response,
        body: Optional[str] = None,
    ):
        message = body or raw_response.text
        super().__init__(message, raw_response, body)
        self.data = data
